...
//-
    ...
    int command_exec();
    void piping() ;
    void piping_exec(int c1, int c2, int* f_m_l) ;
            ...
            while (word[i] != NULL){
                ...
                for(k=0; k<i; k++){
                    if(strncmp(word[k], "<", 1) == 0){
                        ...
                        {
                        if (pipe_red.count > MAX_PIPES){
                            return -2 ;
                        }
                        else{
                            pipe_red.p_index[pipe_red.count] = k ;
                            word[k] = NULL ;
                            pipe_red.count ++ ;
                            }
                        }
...
void piping ()
{
    int f_m_l[2] ;  // 0 => first command ;   1 => middle command ;   2 => las command
    int c1_index, c2_index ;              //
    int i ;
    for (i=0; i< pipe_red.count; i++)   // counter to access all pipes
    {

        // Indicating the first command
        if (i == 0)                         // if it's the first pipelining
        {
            f_m_l[0] = 0;
            c1_index = 0 ;
        ...
        {
            f_m_l[0] = 1 ;
            c1_index = pipe_red.p_index[i-1] + 1 ;
        }

        // Indicating the second command
        if (i == pipe_red.count - 1)  // if it's the last pipelining
        {
            f_m_l[1] = 2 ;
            c2_index = pipe_red.p_index[pipe_red.count -1 ] + 1 ;
        }
        else
        {
            f_m_l[1] = 1 ;
            c2_index = pipe_red.p_index[i] + 1 ;
        }
        //printf("%d\t%d\n", pipe_red.count, pipe_red.p_index[0])
        printf("%d\t%d\n", f_m_l[0], c1_index) ;
        printf("%d\t%d\n", f_m_l[1], c2_index) ;
        piping_exec(c1_index, c2_index, f_m_l) ;
    }
}
    ...
    if(pid_1 ==0)
        {  //child
            ...
            if (pid_2>0){     // parent of the child
                ...
                {
                    close(local_pipe[0]) ;
                    dup2(global_pipe[1], STDOUT_FILENO) ;
                }
                else if(f_m_l[1] == 2 && out_red.count>0  && out_red.index > pipe_red.p_index[pipe_red.count - 1])
                {
                    output_redirecting();
                }
                ...
                {
                    close(global_pipe[1]) ;
                    dup2(global_pipe[0], STDIN_FILENO) ;
                }
                else    // f_m_l[0] == 0
                {
                    close(local_pipe[1]) ;
                    dup2(local_pipe[0], STDIN_FILENO) ;
                }

                execvp(word[c2], &word[c2]) ;
                exit(0) ;
            }
            else
                {     // child of the child
                ...
                if (f_m_l[0] == 0)
                    {
                    if( f_m_l[0] == 0  && in_red.count>0 && in_red.index < pipe_red.p_index[0])
                    {
                        input_redirecting() ;
                    }
                    close(local_pipe[0]);
                    dup2(local_pipe[1], STDOUT_FILENO) ;
                    execvp(word[c1], &word[c1]) ;
                    }
                exit(0) ;
                }
        }
